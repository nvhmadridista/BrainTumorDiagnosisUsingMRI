...
class Predictor:
    ...
    def predict(self, image_bytes: bytes) -> Dict[str, Any]:
        """
        Prediction using TorchScript model.

        Returns:
            Dict with predicted_class, confidence_scores, class_names
        """
...
class GradCAMPredictor:
    def __init__(self, checkpoint_path: Path, num_classes: int = 4) -> None:
        self.device = get_device()
    ...
    def gradcam(self, image_bytes: bytes, target_index: int | None = None) -> Dict[str, Any]:
        """
        Generate Grad-CAM for the given image. If target_index is None,
        it will first predict and use the top-1 class as target.

        Returns a dict with base64 PNGs for overlay and heatmap, and layer name.
        """
        pil_img = load_image_from_bytes(image_bytes)
        input_tensor = self.preprocess_image(pil_img)
        ...
        heatmap = self.cam.generate_cam(input_tensor, target_index)
        overlay = overlay_heatmap(heatmap, resized_pil)

        return {
            "gradcam_overlay": pil_to_base64_png(overlay),
            "gradcam_heatmap": pil_to_base64_png(Image.fromarray((heatmap * 255).astype("uint8"))),
            "gradcam_layer": "stage3",
        }